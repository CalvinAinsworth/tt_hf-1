...
int main(int agrc, char *argv[])
{
  ...
  int draw_min_dR_btag_to_btag = draw_n_hists(h_min_dR_btag_to_btag, discr_legend_titles, "#bf{#DeltaR_{min}(btag, other btag)}", "pl/min_dR_btag_to_btag", true, 0, 0.2);

  // dR_min(btag, not btag)
  std::vector<TH1*> h_min_dR_btag_to_jet = {h_min_dR_btag_from_top_to_jet, h_min_dR_btag_not_from_top_to_jet};
  int draw_min_dR_btag_to_jet = draw_n_hists(h_min_dR_btag_to_jet, discr_legend_titles, "#bf{#DeltaR_{min}(btag, not btag)}", "pl/min_dR_btag_to_not_b", true, 0, 0.15);

  // dR_min(btag, lep)
  std::vector<TH1*> h_min_dR_btag_to_lep = {h_min_dR_btag_from_top_to_lep, h_min_dR_btag_not_from_top_to_lep};
  int draw_min_dR_btag_to_lep = draw_n_hists(h_min_dR_btag_to_lep, discr_legend_titles, "#bf{#DeltaR_{min}(btag, lep)}", "pl/min_dR_btag_to_lep", true, 0, 0.25);

  // m(btag, closest lep)
  std::vector<TH1*> h_m_lep_btag = {h_m_lep_btag_from_top_min_dR, h_m_lep_btag_not_from_top_min_dR};
  int drw_m_lep_btag = draw_n_hists(h_m_lep_btag, discr_legend_titles, "#bf{m(btag, closest lep)}", "pl/m_btag_closest_lep", true, 0, 0.06);

  // m_min(btag, lep)
  std::vector<TH1*> h_min_m_lep_btag = {h_min_m_lep_btag_from_top, h_min_m_lep_btag_not_from_top};
  int draw_min_m_lep_btag = draw_n_hists(h_min_m_lep_btag, discr_legend_titles, "#bf{m_{min}(btag, lep)}", "pl/m_min_btag_lep", true, 0, 0.07);

  // m_max(btag, lep) 
  std::vector<TH1*> h_max_m_lep_btag = {h_max_m_lep_btag_from_top, h_max_m_lep_btag_not_from_top};
  int draw_max_m_lep_btag = draw_n_hists(h_max_m_lep_btag, discr_legend_titles, "#bf{m_{max}(btag, lep)}", "pl/m_max_btag_lep", true, 0, 0.07);


  // jet_pt
  std::vector<TH1*> h_all_jets_pt = {h_all_jets_from_top_pt, h_all_jets_not_from_top_pt};
  int draw_all_jets_pt = draw_n_hists(h_all_jets_pt, discr_legend_titles, "#bf{p_{T}^{jets}}", "pl/all_jets_pt", true, 0, 0.4);

  // jet_eta
  std::vector<TH1*> h_all_jets_eta = {h_all_jets_from_top_eta, h_all_jets_not_from_top_eta};
  int draw_all_jets_eta = draw_n_hists(h_all_jets_eta, discr_legend_titles, "#bf{jet #eta}", "pl/all_jets_eta", true, 0, 0.1);

  // jet_phi
  std::vector<TH1*> h_all_jets_phi = {h_all_jets_from_top_phi, h_all_jets_not_from_top_phi};
  int draw_all_jets_phi = draw_n_hists(h_all_jets_phi, discr_legend_titles, "#bf{jet #phi}", "pl/all_jets_phi", true, 0, 0.1);

  // btag invariant mass
  std::vector<TH1*> h_m_btag = {h_m_btag_top, h_m_btag_other};
  int draw_m_btag = draw_n_hists(h_m_btag, discr_legend_titles, "#bf{m(btag)}", "pl/m_btag", true, 0, 0.12);
  ...
  int draw_m_btags_pairs = draw_n_hists(h_m_btags_pairs, m_btags_pairs_legend, "#bf{m(btag, btag)}", "pl/m_btag_btag", true, 0, 0.1);
  std::vector<TH1*> h_m_min_btags_pairs = {h_m_min_btag_top_btag_top, h_m_min_btag_top_btag_other, h_m_min_btag_other_btag_other};
  int draw_m_min_btags_pairs = draw_n_hists(h_m_min_btags_pairs, m_btags_pairs_legend, "#bf{m_{min}(bteg, btag)}", "pl/m_min_btag_btag", true, 0, 0.1);
  std::vector<TH1*> h_m_max_btags_pairs = {h_m_max_btag_top_btag_top, h_m_max_btag_top_btag_other, h_m_max_btag_other_btag_other};
  int draw_m_max_btags_pairs = draw_n_hists(h_m_max_btags_pairs, m_btags_pairs_legend, "#bf{m_{max}(btag, btag)}", "pl/m_max_btag_btag", true, 0, 0.1);
